...
Command-specific options:
  install        [--version VERSION|--backend-image IMAGE --frontend-image IMAGE --redis-image IMAGE|--bundle PATH]
...
removed_command_message() {
  ...
  case "$command" in
    init|deploy)
      printf "Command '%s' was removed. Copy 'scripts/deploy/templates/riskhub.env.example' and follow 'scripts/deploy/templates/secrets/README.md', then run './scripts/deploy.sh install'.\n" "$command"
      ;;
    secrets-init|secrets-edit|secrets-check)
      printf "Command '%s' was removed. Manage '/etc/riskhub/riskhub.env' from 'scripts/deploy/templates/riskhub.env.example' and '/etc/riskhub/secrets/*' from 'scripts/deploy/templates/secrets/README.md', then run './scripts/deploy.sh doctor' to validate the deployment.\n" "$command"
      ;;
