#!/usr/bin/env bash
# Python planning entrypoint: prod_readiness_audit.cli
set -euo pipefail
...
run_cmd "p4_syft_backend" true 1800 "$ROOT_DIR" "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v '$REPORTS_DIR':/out anchore/syft:v1.42.3@sha256:5999d209a342e55e9edf70bf8930fb5b86d8f2a783fa401178372c50e21b1d36 '$BACKEND_IMAGE_UPGRADE' -o json=/out/sbom-backend.json"
run_cmd "p4_grype_backend" true 1800 "$ROOT_DIR" "docker run --rm -v '$ROOT_DIR':/repo -w /repo anchore/grype:v0.110.0@sha256:af65fbc0c664691067788fe95ff88760b435543e45595eb2ca6f102fc476fbe1 sbom:/repo/tests/results/prod/prod-readiness-audit-$RUN_ID/reports/sbom-backend.json --config /repo/backend/security/grype-ignore.yaml -o json=/repo/tests/results/prod/prod-readiness-audit-$RUN_ID/reports/grype-backend.json"
