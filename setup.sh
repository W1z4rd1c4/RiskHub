...
usage() {
  cat <<'EOF'
Usage: ./setup.sh [options] [-- <passthrough args>]
...
Examples:
  ...
  ./setup.sh --mode dev                # bring up dev docker stack + base seed
  ./setup.sh --mode test               # wipe and seed deterministic E2E data
  ./setup.sh --mode prod               # run the production guided installer
  ./setup.sh --mode test --yes --dry-run -- --no-build
EOF
}
