...
class TestKRIPeriodProtection:
    ...
    def test_latest_closed_period_excludes_future(self):
        ...
        for frequency in [
            ...
            period_start, period_end = KRIHistoryService.latest_closed_period_for_date(today, frequency)
    ...
    def test_core_bug_fix_latest_closed_vs_current_period(self):
        ...
        _, current_period_end = KRIHistoryService.period_bounds_for_date(jan_10, KRIFrequency.quarterly.value)
        assert current_period_end == date(2026, 3, 31), "Current period should be Q1 2026"

        # Latest CLOSED period - should be Q4 2025
        _, latest_closed_end = KRIHistoryService.latest_closed_period_for_date(jan_10, KRIFrequency.quarterly.value)
        ...
        assert (
            latest_closed_end != current_period_end
        ), "latest_closed_end must differ from current_period_end when in open period"
        assert latest_closed_end < current_period_end, "latest_closed_end must be before current_period_end"
    ...
    def test_is_period_end_boundary_validates_correctly(self):
        ...
        # Valid period ends
        assert KRIHistoryService.is_period_end_boundary(date(2026, 3, 31), KRIFrequency.quarterly.value) is True
        assert KRIHistoryService.is_period_end_boundary(date(2026, 1, 31), KRIFrequency.monthly.value) is True

        # Invalid - mid-month is not a valid monthly period end
        assert KRIHistoryService.is_period_end_boundary(date(2026, 1, 15), KRIFrequency.monthly.value) is False
        # Invalid - mid-quarter is not a valid quarterly period end
        assert KRIHistoryService.is_period_end_boundary(date(2026, 2, 28), KRIFrequency.quarterly.value) is False
