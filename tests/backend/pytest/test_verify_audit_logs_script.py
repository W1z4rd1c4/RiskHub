...
# Add backend scripts directory to path for import
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "backend" / "scripts"))
from verify_audit_logs import verify_audit_log, verify_log_separation
...
class TestLogSeparation:
    ...
    def test_clean_separation_passes(self, tmp_path):
        ...
        audit_log.write_text(
            json.dumps({"timestamp": "2024-01-01T12:00:00+00:00", "level": "info", "event": "login", "logger": "audit"})
            + "\n"
        )
