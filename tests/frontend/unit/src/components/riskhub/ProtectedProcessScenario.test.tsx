...
describe('protected Process approval scenario', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        vi.mocked(riskHubApi.getApprovalScenarios).mockResolvedValue([{
            ...
            description: 'Approval for protected Process edits',
    ...
    it('shows immutable policy and restricts choices to Risk Manager/CRO', async () => {
        renderPanel();
    ...
    it('keeps the fixed-policy explanation for older boolean-only responses', async () => {
        vi.mocked(riskHubApi.getApprovalScenarios).mockResolvedValue([{
            id: 84,
            key: 'protected_process_edit',
            display_name: 'Protected Process mutations',
