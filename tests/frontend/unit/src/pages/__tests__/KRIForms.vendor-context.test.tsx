...
describe('KRINewPage vendor context', () => {
    beforeEach(() => {
        ...
        mockGetVendor.mockResolvedValue({ id: 12, name: 'Vendor Twelve' });
    });

    it('passes vendor context and back label through to the KRI form', async () => {
        render(<KRINewPage />);

        expect(screen.getByTestId('kri-back-label')).toHaveTextContent(/Back to vendor|Zpět na dodavatele/i);
