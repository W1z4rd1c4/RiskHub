...
- Current blocker (verified 2026-03-29): the shared demo-login helper in [`tests/frontend/e2e/helpers/login.ts`](./helpers/login.ts) still waits for `http://localhost:5173/...`, so Docker-origin runs with `FRONTEND_URL=http://localhost` currently fail after successful redirect until that helper is updated.
