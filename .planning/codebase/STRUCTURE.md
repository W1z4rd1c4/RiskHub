...
- `backend/app/api/v1/endpoints/` - 165 Python modules/packages (measured git-tracked `*.py` snapshot; extensively split into subrouters for maintainability)
...
- `tests/backend/pytest/` - 452 tracked test files (414 Python)
...
### Primary subdirectories
- `frontend/src/pages/` - 226 tracked files (measured git-tracked snapshot; route-level pages + colocated helpers/tests and shared detail/admin/user workflow modules)
...
- `tests/frontend/e2e/` - 56 E2E specs among 96 tracked files (measured git-tracked `*.spec.ts` snapshot; domain-focused test suites plus setup/helpers/fixtures and the dialog render-site harness)
