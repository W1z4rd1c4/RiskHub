...
- `backend/app/api/v1/endpoints/` - 164 Python modules/packages (measured git-tracked `*.py` snapshot; extensively split into subrouters for maintainability)
...
- `tests/backend/pytest/` - 451 tracked test files (413 Python)
...
### Primary subdirectories
- `frontend/src/pages/` - 224 tracked files (measured git-tracked snapshot; route-level pages + colocated helpers/tests and shared detail/admin/user workflow modules)
- `frontend/src/components/` - 311 tracked files (measured git-tracked snapshot; components + tests, including split linking, remediation, questionnaire workflow state, governance, dashboard, KRI modal, and control-loading-overlay modules)
...
- `tests/frontend/e2e/` - 55 E2E specs among 95 tracked files (measured git-tracked `*.spec.ts` snapshot; domain-focused test suites plus setup/helpers/fixtures and the dialog render-site harness)
...
*Structure audit refreshed on 2026-07-16*
