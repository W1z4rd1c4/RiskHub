...
- `tests/backend/pytest/` - 462 tracked test files (424 Python)
...
- `frontend/src/components/` - 313 tracked files (measured git-tracked snapshot; components + tests, including split linking, remediation, questionnaire workflow state, governance, dashboard, KRI modal, and control-loading-overlay modules)
...
- `tests/frontend/e2e/` - 62 E2E specs among 103 tracked files (measured git-tracked `*.spec.ts` snapshot; domain-focused test suites plus setup/helpers/fixtures and the dialog render-site harness)
