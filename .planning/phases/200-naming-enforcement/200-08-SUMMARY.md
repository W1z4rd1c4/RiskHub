...
- Verified no drift in:
  - `frontend/src/components/reports/ExportDialog.tsx` (`xlsx|csv` only)
  - `frontend/src/services/reportApi.ts`
  - `frontend/src/services/vendorReportApi.ts` (`xlsx` vendor flow)
...
|---|---|---|
| A (backend) | `cd backend && pytest tests/test_reports_rbac.py -k "export or summary or pdf"` | ✅ `20 passed, 1 deselected` |
| B (backend) | `cd backend && pytest tests/test_vendor_reports.py` | ✅ `3 passed` |
| C (backend) | `cd backend && pytest tests/api/v1/test_reports_audit.py` | ✅ `5 passed` |
| D (frontend e2e) | `cd frontend && CI=1 npx playwright test --project=chromium --workers=1 --retries=0 e2e/risks.spec.ts e2e/controls.spec.ts` | ✅ `10 passed` |
| E (frontend e2e) | `cd frontend && CI=1 npx playwright test --project=chromium --workers=1 --retries=0 e2e/kris.spec.ts e2e/vendors.spec.ts` | ✅ `9 passed` |
...
- `backend/tests/test_reports_rbac.py`
- `backend/tests/api/v1/test_reports_audit.py`
- `.planning/phases/200-naming-enforcement/200-08-SUMMARY.md`
- `.planning/ROADMAP.md`
- `.planning/STATE.md`
