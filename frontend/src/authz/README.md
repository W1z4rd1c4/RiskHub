...
`BusinessRouteGuards.ts` exports `createBusinessRouteGuard`, a typed factory whose key parameter is restricted to boolean fields on `Authz` via the local `BoolKeys` type. The named route guards should stay as `export const` bindings to that factory so route semantics remain centralized.
