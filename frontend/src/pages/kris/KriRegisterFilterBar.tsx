...
export function KriRegisterFilterBar({
    ...
    return <RegisterListToolbar
        ...
        lifecycleControl={<div className="flex flex-wrap gap-3">
            ...
            <label className="flex h-10 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-foreground"><input type="checkbox" checked={filters.timeliness_status === 'due_soon'} onChange={(event) => onFilterChange('timeliness_status', event.target.checked ? 'due_soon' : '')} className="accent-accent" />{t('filters.due_soon')}</label>
        </div>}
    ...
    >
        <div className="flex flex-wrap gap-2 md:col-span-2 xl:col-span-3">
            <Button variant="ghost" size="compact" data-testid="kris-status-filter-all" onClick={onClearAll} className={`font-bold uppercase ${filters.lifecycle === 'active' && !filters.monitoring_status && !filters.timeliness_status ? 'bg-accent text-accent-foreground' : 'bg-white/5 text-muted-foreground'}`}>{t('filters.all')}</Button>
            {DEFAULT_MONITORING.map((status) => <Button key={status} variant="ghost" size="compact" data-testid={`kris-status-filter-${status}`} onClick={() => onFilterChange('monitoring_status', status)} className={`font-bold uppercase ${filters.lifecycle === 'active' && filters.monitoring_status === status ? 'bg-accent text-accent-foreground' : 'bg-white/5 text-muted-foreground'}`}>{t(`monitoring.${status}`)}</Button>)}
            <Button variant="ghost" size="compact" data-testid="kris-status-filter-due_soon" onClick={() => onFilterChange('timeliness_status', 'due_soon')} className={`font-bold uppercase ${filters.lifecycle === 'active' && filters.timeliness_status === 'due_soon' ? 'bg-accent text-accent-foreground' : 'bg-white/5 text-muted-foreground'}`}>{t('filters.due_soon')}</Button>
            <Button variant="ghost" size="compact" data-testid="kris-status-filter-archived" onClick={() => onFilterChange('lifecycle', 'archived')} className={`font-bold uppercase ${filters.lifecycle === 'archived' ? 'bg-accent text-accent-foreground' : 'bg-white/5 text-muted-foreground'}`}>{t('filters.archived')}</Button>
        </div>
        {activeKeys.map((key) => <div key={key} className="relative rounded-xl border border-white/10 bg-white/[0.025] p-3 pr-12"><Button variant="secondary" size="iconCompact" aria-label={t('register.filters.remove', { label: labels[key] })} onClick={() => remove(key)} className="absolute right-2 top-2"><X aria-hidden="true" /></Button>{renderOptional(key)}</div>)}
    </RegisterListToolbar>;
}
