...
export function IssuesFilterBar({ facets, filters, isLoading, onClearAll, onFilterChange, onRefresh, onSearchChange, search }: Props) {
    ...
    return <RegisterListToolbar
        ...
        lifecycleControl={<div className="flex flex-wrap gap-3">
            ...
            <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 text-sm text-foreground"><input type="checkbox" checked={filters.include_closed} onChange={(event) => onFilterChange('include_closed', event.target.checked)} className="accent-accent" />{t('filters.include_closed')}</label>
        </div>}
        ...
        testIdPrefix="issues"
    >
        <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.025] p-3 text-sm text-foreground"><input type="checkbox" checked={filters.overdue} onChange={(event) => onFilterChange('overdue', event.target.checked)} className="accent-accent" />{t('filters.overdue_only')}</label>
        <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.025] p-3 text-sm text-foreground"><input type="checkbox" checked={filters.exclude_active_exceptions} onChange={(event) => onFilterChange('exclude_active_exceptions', event.target.checked)} className="accent-accent" />{t('filters.exclude_active_exceptions')}</label>
        {activeKeys.map((key) => <div key={key} className="relative rounded-xl border border-white/10 bg-white/[0.025] p-3"><button type="button" aria-label={t('register.filters.remove', { label: labels[key] })} onClick={() => remove(key)} className="absolute right-2 top-2 inline-flex min-h-6 min-w-6 items-center justify-center rounded"><X className="h-3.5 w-3.5" aria-hidden="true" /></button><ThemedSelect value={String(filters[key] ?? '')} onValueChange={(value) => key === 'remediation_status' ? onFilterChange('remediation_status', value as IssueRegisterFilters['remediation_status']) : onFilterChange(key, value ? Number(value) : null)} allowEmpty emptyLabel={t('common:filters.all')} triggerAriaLabel={labels[key]} options={withCounts(facetFor(key)).map((option) => key === 'remediation_status' ? { ...option, label: t(`remediation_status.${option.value}`, option.label) } : option)} /></div>)}
    </RegisterListToolbar>;
}
