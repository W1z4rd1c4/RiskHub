...
export function IssuesFilterBar({ facets, filters, isLoading, onClearAll, onFilterChange, onRefresh, onSearchChange, search }: Props) {
    ...
    return <RegisterListToolbar
        ...
        lifecycleControl={<div className="flex flex-wrap gap-3">
            ...
            <label className="flex h-10 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-foreground"><input type="checkbox" checked={filters.include_closed} onChange={(event) => onFilterChange('include_closed', event.target.checked)} className="accent-accent" />{t('filters.include_closed')}</label>
        </div>}
