...
export function ControlRegisterFilterBar({ facets, filters, isLoading, onClearAll, onFilterChange, onRefresh, onSearchChange, search }: Props) {
    ...
    return <RegisterListToolbar activeFilterCount={chips.length}
        ...
        lifecycleControl={<div className="flex gap-3"><ThemedSelect value={filters.lifecycle} onValueChange={(value) => onFilterChange('lifecycle', value as ControlLifecycleFilter)} triggerTestId="controls-lifecycle-filter-trigger" contentTestId="controls-lifecycle-filter-content" optionTestIdPrefix="controls-lifecycle-filter-option" options={['active', 'archived', 'all'].map((value) => ({ value, label: t(`register.lifecycle.${value}`) }))} /><ThemedSelect value={filters.monitoring_status} onValueChange={(value) => onFilterChange('monitoring_status', value as ControlRegisterFilters['monitoring_status'])} allowEmpty emptyLabel={t('filters.all_statuses')} triggerAriaLabel={t('filters.all_statuses')} triggerTestId="controls-status-filter-trigger" contentTestId="controls-status-filter-content" optionTestIdPrefix="controls-status-filter-option" options={(facets.monitoring_status?.length ? facets.monitoring_status : ['new', 'needs_review', 'failed', 'passed'].map((value) => ({ value, label: value, count: 0, selected: false, disabled: false }))).map((option) => ({ value: option.value, label: facets.monitoring_status?.length ? `${t(`monitoring.${option.value}`, option.label)} (${option.count})` : t(`monitoring.${option.value}`, option.label), disabled: option.disabled }))} /></div>}
