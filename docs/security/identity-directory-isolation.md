...
| Installer status/doctor/repair/preflight | `scripts/install_lib/{status,doctor,lifecycle}.py`, `scripts/prod/{status,preflight,bootstrap_db}.sh`, deployment renderer | The shared renderer and target adapters select Entra/native configuration and bootstrap. Operator status/doctor expose profile/MFA policy, disabled-directory applicability, delivery degradation, security unavailability and pending onboarding; repair never alters identity authority. Native production admission remains closed until #208. |
