...
2. Run migrations as a dedicated step:
   - Docker Compose: `alembic upgrade head` in an ephemeral backend container
   - Kubernetes: a `Job`/hook that runs `alembic upgrade head`
3. Roll out the backend after migrations succeed.
