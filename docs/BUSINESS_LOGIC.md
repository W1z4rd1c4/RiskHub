...
- Flow metrics for the actual in-progress quarter use equal elapsed current and comparison windows. Completed selected quarters use complete quarter windows. Exact half-open observation ranges and Live sources are returned with each flow metric.
- Snapshot observations identify their exact capture time, metric-definition identifier, and Live, Stored, or Missing source. Stored stock observations must first have the same snapshot type. `QUARTER_END` pairs compare; `MANUAL` pairs compare when both are captured at their exact quarter ends or at equivalent within-quarter positions. Mixed `MANUAL`/`QUARTER_END` pairs are suppressed. Snapshot deltas are also suppressed when an observation is missing, manual positions are genuinely unequal, or metric-definition identifiers differ; named reasons are returned and the UI renders N/A.
- A non-zero value against a zero baseline has a nullable percentage with reason `baseline_zero`; the UI shows “New (from 0)” and the absolute change rather than a fabricated percentage.
