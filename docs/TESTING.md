...
> **Source of Truth**: `tests/backend/pytest/`, `backend/pytest.ini`, `frontend/package.json`, `frontend/playwright.config.ts`, `tests/frontend/contracts/dora-e2e-requirements.json`
...
|---|---|---|
| Native identity MFA policies | `cd backend && pytest ../tests/backend/pytest/test_local_identity.py ../tests/backend/pytest/test_local_identity_postgres.py -q` | Required/optional login, admin-created accounts, password changes/reset, optional MFA enrollment and policy enforcement. Run with a disposable `TEST_DATABASE_URL` for authoritative reset/refresh concurrency coverage under both policies. |
...
| ICT Register DQ bounded-read contract | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_ict_register_dq.py ../tests/backend/pytest/test_rate_limit_components.py ../tests/backend/pytest/test_rate_limit_redis_resilience.py` | Global DQ counts with bounded viewer previews, paginated viewer-scoped detail, revision-aware single-flight evaluation, and the shared 30/min DQ endpoint-family bucket |
...
| Backend Vendor shared register | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_vendor_register_framework.py ../tests/backend/pytest/test_vendors.py ../tests/backend/pytest/test_ict_register_vendor_sub_outsourcing.py` | Complete server-side Vendor filter/search algebra, permission-scoped facets/lookups and By Risk multi-membership, record-owner redaction, active Sub-outsourcing semantics, and localized unpaged standard export |
| Backend CISO/Threat stewardship and shared register | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_ict_register_ciso_stewardship.py ../tests/backend/pytest/test_ict_register_threats.py ../tests/backend/pytest/test_threat_register_framework.py` | Exact CISO least privilege, active-CISO assignment, safe projection, orphan handling, canonical categories, shared filters/facets/lookups, active-CISO ordinary Steward discovery with selected historical-label recovery, readable-Risk multi-membership without scope leakage, localized unpaged export, lifecycle, and links |
| Backend Process ownership/import | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_ict_register_process_ownership.py ../tests/backend/pytest/test_process_schema_contract.py ../tests/backend/pytest/test_ict_register_process_import_and_seed.py ../tests/backend/pytest/migrations/test_process_ownership_migration.py` | Required active Process Owner/Owning Department relationships, cross-Department scope, safe projections, orphan preservation, canonical codes, exact importer resolution/findings, deterministic seed IDs, and ADR-010 migration behavior |
| Backend governed Process mutations | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_governed_mutation_models.py ../tests/backend/pytest/test_governed_mutation_audit.py ../tests/backend/pytest/test_governed_mutation_outbox.py ../tests/backend/pytest/test_notification_preferences.py ../tests/backend/pytest/test_notification_service.py` | ADR-016 immutable proposal/impact-lock contracts, safe audit adapters, typed transactional-outbox routing, and both default-on semantic notification preferences |
| Backend governed Process extended/relationship (SQLite) | `cd backend && ./venv/bin/pytest -q -m "not postgres" ../tests/backend/pytest/test_governed_process_extended.py ../tests/backend/pytest/test_governed_process_relationships.py` | Fast behavioral contract for protected create/archive/relationship intake and resolution, malformed-envelope expiry, queue/privacy projections, safe relationship audits, and lock release on the default SQLite harness |
| Backend governed Process extended/relationship (Postgres) | `cd backend && TEST_DATABASE_URL=postgresql+asyncpg://riskhub:riskhub_dev@localhost:5432/riskhub_test ./venv/bin/pytest -q -m postgres ../tests/backend/pytest/test_governed_process_extended.py ../tests/backend/pytest/test_governed_process_relationships.py` | Authoritative row/advisory-lock ordering, concurrent create/link/authority-change serialization, atomic primary swaps, and reference-drift rollback against the dedicated Postgres test database |
| Backend Asset responsibility/import | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_ict_register_asset_ownership.py ../tests/backend/pytest/test_asset_schema_contract.py ../tests/backend/pytest/test_ict_register_asset_import_and_seed.py ../tests/backend/pytest/migrations/test_asset_ownership_migration.py` | Required active Business Owner/ICT Owner/Owning Department relationships, same-user and cross-Department assignment, record-specific read/update without archive, role-specific orphan preservation, atomic Governance reassignment, safe projections, and canonical codes/import mappings |
| Backend governed Asset mutations | `cd backend && ./venv/bin/pytest -q -m "not postgres" ../tests/backend/pytest/test_governed_asset_mutations.py ../tests/backend/pytest/test_governed_mutation_outbox.py ../tests/backend/pytest/test_notifications.py ../tests/backend/pytest/test_ict_register_asset_ownership.py` | Protected Asset create/edit/archive and relationship intake/resolution, immutable proposal identity, queue/privacy projections, notifications, pending locks, and approved-truth preservation on the default SQLite harness |
| Backend governed Asset migration rehearsal (Postgres) | `cd backend && TEST_DATABASE_URL=postgresql+asyncpg://riskhub:riskhub_dev@localhost:5432/riskhub_test ./venv/bin/pytest -q -m postgres ../tests/backend/pytest/migrations/test_governed_asset_migration_rehearsal.py` | ADR-010 forward-upgrade, rollback, and replay rehearsal for governed Asset schema changes against the dedicated Postgres test database |
| Backend governed Vendor mutations | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_governed_vendor_mutations.py ../tests/backend/pytest/test_governed_vendor_cascade.py` | Protected Vendor/Contract/Sub-outsourcing/link intake and resolution, immutable approved truth, pending locks, safe projections, notifications, and Process/Asset-to-Vendor Composite impact |
| Backend governed Vendor migration rehearsal (Postgres) | `cd backend && TEST_DATABASE_URL=postgresql+asyncpg://riskhub:riskhub_dev@localhost:5432/riskhub_test ./venv/bin/pytest -q -m postgres ../tests/backend/pytest/migrations/test_governed_vendor_migration_rehearsal.py` | ADR-010 forward upgrade, rollback, and replay rehearsal for Vendor approval resource/version/schema changes |
...
| Backend access/Risk Hub config | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_access_management.py ../tests/backend/pytest/test_users.py ../tests/backend/pytest/test_riskhub_roles.py ../tests/backend/pytest/test_riskhub_departments.py ../tests/backend/pytest/test_riskhub_risk_types.py ../tests/backend/pytest/test_admin_sessions.py` | Access split policy, Risk Hub collection and row capability metadata, inactive config edit guards, admin session workflow, atomic role permission replacement, department manager validation, and delete blockers |
...
| Frontend #158 collection identity/detail unit contract | `cd frontend && npm run test:run -- ../tests/frontend/unit/src/pages/shared/collectionPageState.test.ts ../tests/frontend/unit/src/pages/shared/eightRegisterCollectionIdentity.test.tsx ../tests/frontend/unit/src/pages/shared/eightRegisterUrlHistory.test.tsx ../tests/frontend/unit/src/pages/detail/useDetailQuery.test.tsx ../tests/frontend/unit/src/pages/detail/useDetailQueryMigration.test.tsx` | Query-keyed same-query stale retention, different-query masking/reset, A-to-B-to-A and late-response fencing across all eight registers, plus generic non-leaky detail 403/404 classification and cache clearing, protected no-retry behavior, and Back-only invalid-ID recovery |
| Frontend #158 truthful request-outcome unit contract | `cd frontend && npm run test:run -- ../tests/frontend/unit/src/pages/__tests__/NotificationsPage.test.tsx ../tests/frontend/unit/src/components/notifications/__tests__/NotificationBell.test.tsx ../tests/frontend/unit/src/pages/__tests__/ApprovalsPage.questionnaire-outcomes.test.tsx ../tests/frontend/unit/src/pages/__tests__/ActivityLogPage.test.tsx ../tests/frontend/unit/src/pages/__tests__/AuditTrailPage.execution-status.test.tsx ../tests/frontend/unit/src/components/__tests__/ExecutionHistory.test.tsx ../tests/frontend/unit/src/components/vendors/VendorLinkedEntitiesTab.test.tsx` | Initial failure, same-query stale retry, cross-query non-reuse, protected 403 clearing, mutation/query races, and error recovery for the checkpoint-4 collection/request surfaces |
| Frontend #158 browser evidence (desktop PASS) | `cd frontend && npx playwright test -c playwright.config.ts --project=ci --workers=1 --retries=0 ../tests/frontend/e2e/ux158-truthful-outcomes.spec.ts && npx playwright test -c playwright.config.ts --project=ci --workers=1 --retries=0 ../tests/frontend/e2e/eight-register-parity.spec.ts ../tests/frontend/e2e/dialog-render-sites.spec.ts && npx playwright test -c playwright.config.ts --project=ci --workers=1 --retries=0 ../tests/frontend/e2e/accessibility-smoke.spec.ts ../tests/frontend/e2e/dora-ux-stateful-a11y.spec.ts ../tests/frontend/e2e/dialog-render-sites.spec.ts ../tests/frontend/e2e/theme-contrast-matrix.spec.ts && npm run e2e:a11y:results -- ../tests/results/frontend/playwright/test-results/results.json` | Observed with Node 24 on desktop: truthful outcomes 6/6 PASS (52.2s); expanded eight-register parity plus dialog render sites 72/72 PASS (2.9m); canonical four-spec accessibility gate 81/81 PASS (10.3m), followed by result-validator PASS. The parity spec now has 16 cases, including all eight register row-to-detail-to-visible-Back working-set restorations. This is bounded browser evidence, not ticket closure, release readiness, or broader strict-Axe sign-off |
...
| Frontend Vendor shared register | `cd frontend && npm run test:run -- ../tests/frontend/unit/src/pages/vendors/__tests__/VendorRegisterFilterBar.test.tsx ../tests/frontend/unit/src/pages/vendors/__tests__/useVendorsPageState.sharedState.test.tsx ../tests/frontend/unit/src/pages/vendors/__tests__/vendorRegisterConfig.test.ts ../tests/frontend/unit/src/services/vendorApi.collection.test.ts` | Six views, URL-backed search/filter/group/sort state, complete filter controls, lifecycle, collection concurrency and denial clearing, safe lookups, capabilities, and standard-export query construction |
...
| Frontend Asset responsibility | `cd frontend && npm run test:run -- ../tests/frontend/unit/src/pages/assets ../tests/frontend/unit/src/pages/__tests__/AssetDetailPage.ownershipResolution.test.tsx ../tests/frontend/unit/src/components/governance/__tests__/ResolveOrphanModal.test.tsx` | Required searchable Business/ICT owner and Department pickers, same-user/cross-Department behavior, safe detail, edit cache, role-aware Governance lock and atomic resolution, stale lookup protection, and accessibility |
...
| Frontend Threat shared register | `cd frontend && npm run test:run -- ../tests/frontend/unit/src/pages/__tests__/ThreatsPage.presentation.test.ts ../tests/frontend/unit/src/pages/threats/__tests__/ThreatRegisterFilterBar.test.tsx ../tests/frontend/unit/src/pages/threats/__tests__/threatRegisterConfig.test.ts ../tests/frontend/unit/src/pages/threats/__tests__/useThreatsPageState.sharedState.test.tsx ../tests/frontend/unit/src/services/threatApi.collection.test.ts` | Five views, URL-backed search/filter/group/sort state, collection concurrency and denial handling, scoped filter controls, export query construction, and localized presentation |
| Frontend Threat targeted browser | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/threat-register-framework.spec.ts` | Keyboard views, page reset, full filter controls, grouped URL history, readable linked-Risk multi-membership without hidden context, export, CISO capabilities, retry, and access denial |
| Frontend Vendor targeted browser | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/vendor-register-framework.spec.ts` | Six keyboard views, representative full filter algebra, grouped URL/history, two readable Risk memberships without hidden-Risk leakage, filtered export, record-only owner capabilities, retry/access denial, and a populated-state axe scan |
...
| Frontend jsx-a11y strict zero | `cd frontend && npm run lint:a11y` | Every enabled recommended rule is an error; zero findings, empty audit evidence, and zero suppressions required; no writable exception path |
| Frontend axe zero-tolerance smoke (DORA) | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/accessibility-smoke.spec.ts` | Enforce-only axe scan of 27 logical routes × 3 themes (81 empty cells) against `accessibility-axe-baseline.json`; any WCAG-tag violation fails (no capture path) |
| Frontend dialog inventory + unit matrix | `cd frontend && npm run lint:dialog-inventory && npm run test:run -- ../tests/frontend/unit/src/components/dialogInteractionMatrix.test.tsx` | Validates 27 implementation owners, 57 application render sites, 5 non-dialog surfaces, and 30 real owner/variant contract cases |
| Frontend dialog render-site browser matrix | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/dialog-render-sites.spec.ts` | Drives all 57 inventoried production render sites (25 source-owner harnesses + 32 real authenticated routes), plus one exact registry/manifest assertion, through open/name/focus/trap/Escape/restoration; owner-load console/page errors, failed requests, and HTTP error responses are fatal outside exact aborted-query handoffs |
| Frontend E2E | `cd frontend && npm run e2e` | Browser-level regression |
| Frontend DORA E2E collection contract | `cd frontend && npm run e2e:dora:collect` | Dynamically collects the Playwright `ci` project and fail-closed validates the versioned DORA requirement/evidence manifest; no historical total is pinned |
...
| Frontend governed Process create/relationship E2E | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/approval-workflows/governed-process-create.spec.ts ../tests/frontend/e2e/approval-workflows/governed-process-relationships.spec.ts` | Requester/CRO creation, cancellation, archive/restore, and Risk/Asset/Vendor relationship approval flows with approved-truth preservation and deterministic cleanup |
| Frontend governed Asset E2E | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/approval-workflows/governed-asset.spec.ts` | Protected Asset request, pending-change, approval, cancellation, approved-truth, and accessibility behavior through the production UI |
...
| Frontend governed Vendor E2E | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/approval-workflows/governed-vendor.spec.ts` | Protected Vendor accountability rejection and independent approval, unchanged approved truth while pending, approval-gated archive, direct restore, and zero-tolerance pending-state accessibility through the production UI |
...
## Frontend Accessibility Contract (DORA UX)

The DORA frontend-UX remediation added machine-checkable accessibility gates. The CI PR lane
enforces the strict-zero lint policy, collection guard, route/state axe scans, and dialog matrices;
the targeted local commands are `npm run lint` / `lint:a11y` and the `--project=ci` Playwright specs.

- **jsx-a11y direct strict zero.** `frontend/scripts/a11y/jsx-a11y-baseline.mjs` collects every enabled `jsx-a11y/*` finding and requires zero. Every enabled recommended plugin rule is configured as an error; rules the plugin intentionally disables stay `off`. `jsx-a11y-baseline.json` must be well-formed with `count: 0, entries: []`, and `eslint-suppressions.json` must be `{}`. Any finding, malformed/non-empty evidence, suppression, or unsupported write argument fails. There is no anchor, fingerprint, deviation, or update workflow; a future exception mechanism requires a separate policy change and tracked approval.
- **axe direct strict zero.** `tests/frontend/e2e/helpers/axeBaseline.ts` validates the committed JSON as the exact `ci` × three-theme × 27-logical-route matrix (81 cells) with every cell empty, then fails directly on every finding. The 25 CRO cells cover the 20 resting business routes plus Control, Risk, KRI, Department, and Vendor detail identities; the two admin cells cover `/admin` and `/admin/docs`. Detail IDs are resolved at runtime through authenticated list APIs, while evidence keeps the stable `/:id` logical key and records the resolved URL. The scans use a 1440×900 desktop viewport. `accessibility-smoke.spec.ts` pins `wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, and `wcag22aa`, analyzes the full rendered page, applies no impact filter, and disables no rules.
- **Dialog source inventory + unit matrix.** `tests/frontend/contracts/dialog-surfaces.json` is the canonical two-level descriptor: 27 implementation owners, 57 concrete application render sites, and 5 non-dialog surfaces. `validate-dialog-inventory.mjs` parses source and fails on owner/render-site/case drift. `dialogInteractionMatrix.test.tsx` executes 30 unique real owner/variant cases, waits for deterministic loaded sentinels, uses exact MSW handlers, treats unexpected requests/errors/console/`act` output as failures, and runs axe after load. The loading case mounts production `ControlRiskLoadingOverlay`, not copied markup.
- **Every render site in a browser.** `dialog-render-sites.spec.ts` reads the same manifest and creates 57 render-site cases plus one exact registry/manifest assertion. Twenty-five component-owned sites mount their production source parents through owner harnesses; 32 page-owned sites drive real authenticated application routes. No driver mounts a leaf dialog directly. Live monitoring starts after authentication and before owner navigation; it retains console/page errors through owner load/open/close and fails request failures or HTTP error responses. The only allowlisted failures are exact `net::ERR_ABORTED` handoffs for the login shell summary, dashboard overview route transitions, mocked governance overview refresh, and admin health/jobs/outbox queries cancelled during section changes. Each render-site case verifies the opener, role/name, initial focus, forward/reverse containment, Escape close, and focus restoration. `dora-ux-stateful-a11y.spec.ts` retains three-theme representative state scans and proves the layered real-dialog + portalled-`ThemedSelect` contract with two Escapes and zero axe exclusions.
- **CI project and fallback.** `npm run e2e:a11y:collect` fails unless all four accessibility specs (`accessibility-smoke`, `dora-ux-stateful-a11y`, `dialog-render-sites`, and `theme-contrast-matrix`) are collected by `ci`. The E2E workflow always executes `--project=ci`; if system Chrome is missing it unsets `PLAYWRIGHT_CHROMIUM_CHANNEL` and uses bundled Chromium rather than changing projects.
