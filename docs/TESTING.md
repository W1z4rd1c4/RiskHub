...
| Frontend dialog render-site browser matrix | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/dialog-render-sites.spec.ts` | Drives all 48 inventoried production render sites through open/name/focus/trap/Escape/restoration with fatal unexpected network/console output |
