...
- **CI project and fallback.** `npm run e2e:a11y:collect` fails unless all three accessibility specs are collected by `ci`. The E2E workflow always executes `--project=ci`; if system Chrome is missing it unsets `PLAYWRIGHT_CHROMIUM_CHANNEL` and uses bundled Chromium rather than changing projects.
