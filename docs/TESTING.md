...
| Frontend a11y baseline/ratchet | `cd frontend && npm run lint:a11y` | Fingerprinted jsx-a11y baseline (`frontend/scripts/a11y/jsx-a11y-baseline.mjs`, `count: 0`) with the exact fail-closed base-ref ratchet; any new, stale, or widened finding fails |
...
| Frontend dialog interaction matrix | `cd frontend && npm run test:run -- ../tests/frontend/unit/src/components/dialogInteractionMatrix.test.tsx` | 26 real dialog/alertdialog surfaces + one non-dialog overlay guard under the seven-point focus/aria/axe contract (27 cases, 0 skipped) |
...
The DORA frontend-UX remediation added machine-checkable accessibility gates. All three are enforced on the CI PR lane; the targeted local commands are `npm run lint` / `lint:a11y` and the `--project=ci` Playwright smoke.

- **jsx-a11y baseline + exact fail-closed ratchet.** `eslint-plugin-jsx-a11y` rules are pinned to `error`, and `frontend/scripts/a11y/jsx-a11y-baseline.mjs` enforces a committed, fingerprinted baseline keyed by `rule | file | line | column`. The baseline is now **empty (`count: 0`)**: a finding not in the baseline fails, and a baseline entry not seen this run also fails, so the baseline may only shrink, never widen. A base-ref ratchet additionally forbids the committed baseline from widening — it resolves `origin/main` when that ref carries `jsx-a11y-baseline.json`, else the committed anchor SHA in `frontend/scripts/a11y/baseline-anchor.json`; if **neither** resolves it **fails closed** (it never skips), so the 0-entry baseline can never be silently re-widened. `npm run lint` chains `eslint .` then `lint:a11y` (`node scripts/a11y/jsx-a11y-baseline.mjs`).
- **axe zero-tolerance smoke.** `tests/frontend/e2e/helpers/axeBaseline.ts` is **enforce-only** — it has no capture/overwrite path, so an axe finding can only be resolved by fixing the app. The committed ledger `tests/frontend/e2e/accessibility-axe-baseline.json` enumerates 11 routes × 3 themes (`riskhub` / `light` / `dark`), including the DORA routes `/ict-register/data-quality` and `/?view=ict-committee`, with every cell an empty `[]` (zero tolerated violations). The stateful `tests/frontend/e2e/accessibility-smoke.spec.ts` scans with pinned WCAG tags (`wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, `wcag22aa`), is **not** filtered by axe impact, and diffs each project→theme→route cell; any finding is NEW drift and fails. The suite is assigned to the `ci` Playwright project only.
- **Dialog interaction matrix.** `tests/frontend/unit/src/components/dialogInteractionMatrix.test.tsx` mounts **26 real dialog/alertdialog surfaces** OPEN through the project render helper and asserts a **seven-point contract** per surface: `aria-modal`, an accessible name (via `aria-labelledby`), initial focus inside, Tab and Shift+Tab focus-trap, an open-state axe sweep (pinned WCAG tags), and Escape-closes-with-focus-restored-to-opener. A 27th case guards the non-dialog busy overlay (`role="status"`, which must not trap focus). The file has **27 `it()` cases, 0 skipped**.
