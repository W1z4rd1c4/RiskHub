...
| Frontend dialog inventory + unit matrix | `cd frontend && npm run lint:dialog-inventory && npm run test:run -- ../tests/frontend/unit/src/components/dialogInteractionMatrix.test.tsx` | Validates 26 implementation owners, 54 application render sites, 5 non-dialog surfaces, and 29 real owner/variant contract cases |
| Frontend dialog render-site browser matrix | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/dialog-render-sites.spec.ts` | Drives all 54 inventoried production render sites (23 source-owner harnesses + 31 real authenticated routes), plus one exact registry/manifest assertion, through open/name/focus/trap/Escape/restoration; owner-load console/page errors, failed requests, and HTTP error responses are fatal outside exact aborted-query handoffs |
