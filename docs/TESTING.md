...
| Backend CISO/Threat stewardship | `cd backend && ./venv/bin/pytest -q ../tests/backend/pytest/test_ict_register_ciso_stewardship.py ../tests/backend/pytest/test_ict_register_threats.py` | Exact CISO least privilege, active-CISO assignment, safe projection, orphan detection and explicit reassignment, canonical categories, Threat lifecycle, and link behavior |
