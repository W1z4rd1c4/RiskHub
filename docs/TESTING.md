...
| Frontend axe zero-tolerance smoke (DORA) | `cd frontend && npx playwright test -c playwright.config.ts --project=ci ../tests/frontend/e2e/accessibility-smoke.spec.ts` | Enforce-only axe scan of 11 routes × 3 themes against the empty `accessibility-axe-baseline.json`; any WCAG-tag violation fails (no capture path) |
...
- **axe direct strict zero.** `tests/frontend/e2e/helpers/axeBaseline.ts` validates the committed JSON as the exact `ci` × three-theme × 11-route matrix with every cell empty, then fails directly on every finding. `accessibility-smoke.spec.ts` pins `wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, and `wcag22aa`, applies no impact filter, and disables no rules.
