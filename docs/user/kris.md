...
1. Create or edit a kri.
2. Link it to the risk it monitors.
3. Set warning and breach thresholds.
4. Submit a period value.
5. Review history.
6. Request a correction when a submitted value is wrong.
...
New values, corrections, and sensitive edits can require review depending on your role and the organization’s rules. If a correction is pending, wait for the decision before submitting another correction for the same point.
...
- Choose thresholds that trigger early enough to act.
- Do not submit placeholder values just to clear an overdue state.
...
Start with [Risks](./risks.md), [Dashboard](./dashboard.md), [Vendors](./vendors.md), [Notifications](./notifications.md), [Activity Log](./activity-log.md). These manuals explain the connected workflows and help you follow the record from signal to action to evidence.
