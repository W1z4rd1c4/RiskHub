...
**Orchestrator role:** Parse arguments, validate phase, research domain (unless skipped or exists), Execute planner process, verify plans with gsd-plan-checker, iterate until plans pass or max iterations reached, present results.

**Why execution steps:** Research and planning burn context fast. Verification uses fresh context. User sees the flow between agents in main context.
</objective>
...
Proceed to execute researcher

### Execute gsd-phase-researcher
...
Fill research prompt and execute:
...
Task(
  ...
  execution step_type="gsd-phase-researcher",
...
Task(
  ...
  execution step_type="gsd-planner",
...
**`## CHECKPOINT REACHED`:**
- Present to user, get response, execute continuation (see step 12)
...
## 10. Execute plan-checker process
...
Fill checker prompt and execute:
...
Task(
  ...
  execution step_type="gsd-plan-checker",
...
Execute gsd-planner with revision prompt:
...
- After planner returns → execute checker again (step 10)
...
- [ ] gsd-phase-researcher used if research needed
- [ ] Existing plans checked
- [ ] gsd-planner used with context (including RESEARCH.md if available)
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gsd-plan-checker used (unless --skip-verify)
- [ ] Verification passed OR user override OR max iterations with user decision
- [ ] User sees status between process steps
