...
> **Comprehensive reference.** Core rules auto-load from `.agent/rules/`. This document provides deep explanations and examples for when you need the full picture.

This document explains how GSD is written so future agent sessions can contribute consistently.

## Core Philosophy

GSD is a **meta-prompting system** where every file is both implementation and specification. Files teach the agent how to build software systematically. The system optimizes for:

- **Solo developer + the agent workflow** (no enterprise patterns)
- **Context engineering** (manage the agent's context window deliberately)
...
allowed-tools: [Read, Write, Bash, Glob, Grep, notify_user or inline question]
---
```
...
**Task types:**
- `type="auto"` — the agent executes autonomously
...
```
@.agent/skills/gsd/workflows/execute-phase.md
@.planning/PROJECT.md
```
...
**@-references are lazy loading signals.** They tell the agent what to read, not pre-loaded content.
...
Use execution steps for autonomous work. Reserve main context for user interaction.

### State Preservation

- `STATE.md` — Living memory across sessions
- `agent-history.json` — execution step tracking for resume
...
Always use notify_user or inline question with concrete options. Never plain text prompts.
...
6. **Solo developer + the agent** — no enterprise patterns
...
11. **notify_user or inline question for all exploration** — always options
