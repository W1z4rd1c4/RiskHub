---
name: gsd-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Used by /gsd:plan-phase orchestrator.
...
- Decompose phases into sequentially-optimized plans with 2-3 tasks each
...
**Automation-first rule:** If the agent CAN do it via CLI/API, the agent MUST do it. Checkpoints are for verification AFTER automation, not for manual work.

## Task Sizing

Each task should take the agent **15-60 minutes** to execute. This calibrates granularity:
...
**The test:** Could a different agent session execute this task without asking clarifying questions? If not, add specificity.
...
Record in `user_setup` frontmatter. Only include what the agent literally cannot do (account creation, secret retrieval, dashboard config).
...
```
Result: All three can run sequentially (Wave 1)
...
## File Ownership for sequential execution
...
# Plan 02 frontmatter (no overlap = can run in same wave)
files_modified: [src/models/product.ts, src/api/products.ts]
```

No overlap -> can run in same wave.
...
Only include what the agent literally cannot do (account creation, secret retrieval, dashboard config).
...
When the agent tries CLI/API and gets auth error, this is NOT a failure - it's a gate.

Pattern: the agent tries automation -> auth error -> creates checkpoint -> user authenticates -> the agent retries -> continues

Authentication gates are created dynamically when the agent encounters auth errors during automation. They're NOT pre-planned.
...
Identify wave grouping opportunities:
- No dependencies = Wave 1 (sequential)
- Depends only on Wave 1 = Wave 2 (sequential)
...
Rules:
1. Same-wave tasks with no file conflicts -> can be sequentially plans
...
- [ ] Wave structure maximizes efficiency
