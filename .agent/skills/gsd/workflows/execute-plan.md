...
Plans are divided into segments by checkpoints. Each segment is routed to optimal execution context (execution step or main).
...
- **Fully autonomous plan** - execute single execution step for entire plan
- execution step gets fresh 200k context, executes all tasks, creates SUMMARY, commits
...
IF segment has no prior checkpoint:
  → execution step (first segment, nothing to depend on)

IF segment follows checkpoint:human-verify:
  → execution step (verification is just confirmation, doesn't affect next work)
...
```
execute task → execute all tasks → SUMMARY → commit → report back
```

**Pattern B: Segmented with verify-only checkpoints**

```
Segment 1 (tasks 1-3): execute task → execute → report back
Checkpoint 4 (human-verify): Main context → you verify → continue
Segment 2 (tasks 5-6): Execute NEW execution step → execute → report back
...
2. Use task instructions with execution step_type="executor":
...
3. After task instructions returns with agent_id:

   a. Write agent_id to current-agent-id.txt:
      echo "[agent_id]" > .planning/current-agent-id.txt

   b. Append execute entry to agent-history.json:
      {
...
4. Wait for execution step to complete

5. After execution step completes successfully:
...
For each autonomous segment:
  execute task with prompt: "Execute tasks [X-Y] from plan at .planning/phases/{phase}-{plan}-PLAN.md. Read the plan for full context and deviation rules. Do NOT create SUMMARY or commit - just execute these tasks and report results."

  Wait for execution step completion
...
**Initialize agent tracking for execution step resume capability.**

Before running any execution steps, set up tracking infrastructure:
...
# Will be populated when execution step starts
...
- This agent can potentially be resumed using task instructions's `resume` parameter
- Present to user: "Previous session was interrupted. Resume agent [ID] or start fresh?"
- If resume: Use task instructions with `resume` parameter set to the interrupted ID
...
- Never remove entries with status "started" (may need resume)
- Keep file under size limit for fast reads

**When to run this step:**
- Pattern A (fully autonomous): Before running the single execution step
- Pattern B (segmented): Before the segment execution loop
- Pattern C (main context): Skip - no execution steps started
</step>
...
2. For each segment in order:
   ...
   A. Determine routing (apply rules from parse_segments):
      - No prior checkpoint? → execution step
      - Prior checkpoint was human-verify? → execution step
      - Prior checkpoint was decision/human-action? → Main context

   B. If routing = execution step:
      ```
      Execute task instructions with execution step_type="executor":
      ...
      **After task instructions returns with agent_id:**

      1. Write agent_id to current-agent-id.txt:
         echo "[agent_id]" > .planning/current-agent-id.txt

      2. Append execute entry to agent-history.json:
         {
           "agent_id": "[agent_id from Task response]",
      ...
      Wait for execution step to complete
      Capture results (files changed, deviations, etc.)

      **After execution step completes successfully:**
   ...
   D. After segment completes (whether execution step or main):
...
3. After ALL segments complete:
   ...
   B. Create SUMMARY.md:
      ...
      - Note which segments were execution steped
...
- Segment 1: No prior checkpoint → execution step ✓
- Checkpoint 4: Verify only → MAIN (required)
- Segment 2: After verify → execution step ✓
- Checkpoint 7: Verify only → MAIN (required)
- Segment 3: After verify → execution step ✓

Execution:
[1] Running execution step for tasks 1-3...
→ execution step completes: 3 files modified, 0 deviations
...
────────────────────────────────────────────────────────
User: "approved"
[3] Running execution step for tasks 5-6...
→ execution step completes: 2 files modified, 1 deviation (added error handling)
[4] Executing checkpoint 7 (human-verify)...
User: "approved"
[5] Running execution step for task 8...
→ execution step completes: 1 file modified, 0 deviations

Aggregating results...

- Total files: 6 modified
- Total deviations: 1
- Segmented execution: 3 execution steps, 2 checkpoints
...
- execution step 1: Fresh 0-30% (tasks 1-3)
- execution step 2: Fresh 0-30% (tasks 5-6)
- execution step 3: Fresh 0-20% (task 8)
...
Use notify_user or inline question:
...
See `.agent/skills/gsd/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See .agent/skills/gsd/references/checkpoints.md for complete checkpoint guidance.
</step>

<step name="checkpoint_return_for_orchestrator">
**When Used by an orchestrator (execute-phase or execute-plan command):**

If you were used via task instructions and hit a checkpoint, you cannot directly interact with the user. Instead, RETURN to the orchestrator with structured checkpoint state so it can present to the user and execute a fresh continuation session.
...
4. Execute a FRESH continuation session with your completed tasks state
...
**How to know if you were started:**

If you're reading this workflow because an orchestrator used you (vs running directly), the orchestrator's prompt will include checkpoint return instructions. Follow those instructions when you hit a checkpoint.

**If running in main context (not started):**
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `.agent/skills/gsd/templates/user-setup.md`.
...
Use .agent/skills/gsd/templates/summary.md for structure.
...
For commit message conventions, see .agent/skills/gsd/references/git-integration.md
</step>
