<purpose>
Execute all plans in a phase using wave-based sequential execution. Orchestrator stays lean by delegating plan execution to execution steps.
</purpose>

<core_principle>
The orchestrator's job is coordination, not execution. Each execution step loads the full execute-plan context itself. Orchestrator discovers plans, analyzes dependencies, groups into waves, runs agents, handles checkpoints, collects results.
</core_principle>
...
Execute each wave in sequence. Autonomous plans within a wave run sequentially.

**For each wave:**

1. **Describe what's being built (BEFORE execution):**
   ...
   **{Plan ID}: {Plan Name}** (if multiple)
   {same format}

   Running {count} analysis pass(es)...
...
2. **Execute all autonomous processes in wave simultaneously:**

   Use task instructions with multiple sequential calls. Each agent gets prompt from execution step-task-prompt template:
   ...
   <execution_context>
   @.agent/skills/gsd/workflows/execute-plan.md
   @.agent/skills/gsd/templates/summary.md
   @.agent/skills/gsd/references/checkpoints.md
   @.agent/skills/gsd/references/tdd.md
   </execution_context>
...
2. **Wait for all agents in wave to complete:**
   ...
   task instructions blocks until each agent finishes. All sequential passes return together.
...
1. **Execute agent for checkpoint plan:**
   ```
   // Execute the following task instructions sequentially:
# {execution step-task-prompt}", execution step_type="general-purpose")
   ```
...
5. **User responds:**
   - "approved" / "done" → Execute continuation process
   - Description of issues → Execute continuation process with feedback
   - Decision selection → Execute continuation process with choice

6. **Execute continuation process (NOT resume):**

   Use the continuation-prompt.md template:
   ```
   Task(
     prompt=filled_continuation_template,
     execution step_type="general-purpose"
   )
   ```
...
7. **continuation session executes:**
...
Resume relies on Platform Team's internal serialization which breaks with sequential tool calls.
Fresh agents with explicit state are more reliable and maintain full context.

**Checkpoint sequentially context:**
If a plan in a sequential wave has a checkpoint:
- execute as normal
- Agent pauses at checkpoint and returns with structured state
- Other sequential passes may complete while waiting
- Present checkpoint to user
- Execute continuation process with user response
...
Verify what actually exists in the code.",
  execution step_type="gsd-verifier"
)
```
