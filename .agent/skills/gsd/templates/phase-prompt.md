...
Template for `.planning/phases/XX-name/{phase}-{plan}-PLAN.md` - executable phase plans optimized for sequential execution.
...
**Must-haves enable verification:** The `must_haves` field carries goal-backward requirements from planning to execution. After all plans complete, execute-phase runs a verification execution step that checks these criteria against the actual codebase.

---

## Wave Grouping vs Sequential

<wave_examples>

**Wave 1 candidates (sequential):**
...
All three run sequentially (Wave 1) - no dependencies, no file conflicts.
...
**Checkpoint behavior in sequential execution:**
...
Purpose: Self-contained user management that can run in same wave to other features.
...
See `.agent/skills/gsd/templates/user-setup.md` for full schema and examples
...
4. Verification execution step checks must_haves against codebase
5. Gaps found → fix plans created → execute → re-verify
6. All must_haves pass → phase complete

See `.agent/skills/gsd/workflows/verify-phase.md` for verification logic.
