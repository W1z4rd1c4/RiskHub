...
**Key elements:**
- `<what-built>`: What the agent automated (deployed, built, configured)
...
**Use ONLY for:**
- **Authentication gates** - the agent tried to use CLI/API but needs credentials to continue (this is NOT a failure)
...
**Key principle:** the agent automates EVERYTHING possible first, only asks human for the truly unavoidable manual step.
...
**Key distinction:** Authentication gates are created dynamically when the agent encounters auth errors during automation. They're NOT pre-planned - the agent tries to automate first, only asks for credentials when blocked.
</type>
</checkpoint_types>
...
```
the agent: Running `vercel --yes` to deploy...
...
the agent: Verifying authentication...
...
**DON'T:**
- Ask human to do work The agent can automate (deploy, create resources, run builds)
- Assume knowledge: "Configure the usual settings" ❌
- Skip steps: "Set up database" ❌ (too vague)
- Mix multiple verifications in one checkpoint (split them)
- Make verification impossible (The agent can't check visual appearance without user confirmation)
...
**Bad placement:**
- Before the agent automates (asking human to do automatable work) ❌
...
```xml
<!-- the agent automates everything -->
<task type="auto">
  <name>Deploy to Vercel</name>
...
### ✅ GOOD: the agent automates, human verifies
...
**Why bad:** The agent has Write tool. This should be `type="auto"`.
...
Checkpoints formalize human-in-the-loop points. Use them when The agent cannot complete a task autonomously OR when human verification is required for correctness.

**The golden rule:** If the agent CAN automate it, the agent MUST automate it.

**Checkpoint priority:**
1. **checkpoint:human-verify** (90% of checkpoints) - the agent automated everything, human confirms visual/functional correctness
...
- Things The agent can verify programmatically (tests pass, build succeeds)
- File operations (The agent can read files to verify)
