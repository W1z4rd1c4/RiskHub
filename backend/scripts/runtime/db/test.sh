...
usage() {
  cat <<'USAGE'
Usage: backend/scripts/runtime/db/test.sh [options]

Destructive compose DB test reset/start flow:
...
confirm_or_die() {
  ...
  if [[ ! -t 0 ]]; then
    die "Refusing to proceed in non-interactive mode without --yes"
  fi
...
log "TEST mode: this will wipe docker-compose volumes (destructive)."
confirm_or_die "Proceed with compose down -v and DB re-initialization?"
