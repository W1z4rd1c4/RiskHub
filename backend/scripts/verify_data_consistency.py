...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Verify data consistency across API endpoints"
    )
    parser.add_argument(
        "--base-url",
        type=str,
...
async def test_data_consistency(base_url: str = "http://localhost:8000", token: str = ""):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            all_risks, reported_total = await fetch_all_paginated(
                client, f"{api_url}/risks", headers, page_size=10
            )
            
            if len(all_risks) == reported_total:
                print(f"   {Colors.GREEN}✓ PASS{Colors.RESET}: Pagination accurate ({len(all_risks)} items, total: {reported_total})")
                passed += 1
            else:
                print(f"   {Colors.RED}✗ FAIL{Colors.RESET}: Fetched {len(all_risks)} but total reported {reported_total}")
                failed += 1
        except Exception as e:
            print(f"   {Colors.RED}✗ ERROR{Colors.RESET}: {e}")
            failed += 1
