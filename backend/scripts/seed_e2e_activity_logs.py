...
async def seed_activity_logs():
    ...
    async with session_context(get_settings()) as db:
        ...
        result = await db.execute(select(ActivityLog).where(ActivityLog.description.contains("E2E-SEED")))
        ...
        for i, (entity_type, action, desc) in enumerate(ENTITY_ACTIONS):
            ...
            if action == ActivityAction.UPDATE:
                changes = {"description": {"old": "Original value", "new": "Updated by E2E seed"}}
