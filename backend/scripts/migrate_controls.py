...
Migrates controls from Katalog kontrol Provoz_06 2025.xlsx to the database.
...
async def migrate_controls():
    ...
    excel_path = Path(__file__).resolve().parents[1] / "data" / "seed_assets" / "Katalog kontrol Provoz_06 2025.xlsx"
    ...
    async with session_context(get_settings()) as session:
        ...
        # Get OPS department
        dept_result = await session.execute(select(Department).where(Department.code == "OPS"))
        ...
        for row_idx in range(2, sheet.max_row + 1):
            ...
            _control_owner = str(sheet.cell(row=row_idx, column=8).value or "").strip()
