"""sync asset permissions for existing databases

ICT Register Asset register (issue #43): existing databases were seeded
before the `assets` resource existed, and the startup seed only populates
...
q4r5s6t7u8v9 for processes), this data migration idempotently ensures the
`assets` permission rows exist and grants them to the default roles exactly
as ``app/db/rbac_seed_contract.py`` declares: risk_manager holds ``assets:*``
and every role holding ``vendors:read`` gains ``assets:read``. The CRO
wildcard row is re-ensured and CRO additionally receives explicit asset rows
(the 18c1d2e3f4a7 precedent). Existing custom grants are never removed.
...
# kept in parity by tests/backend/pytest/test_ict_register_assets.py.
ASSET_PERMISSIONS: tuple[dict[str, str], ...] = (
    {"resource": "assets", "action": "read", "description": "View ICT Register assets"},
    {"resource": "assets", "action": "write", "description": "Create/edit ICT Register assets"},
    {"resource": "assets", "action": "delete", "description": "Archive/restore ICT Register assets"},
)

# Mirrors RBAC_ROLE_PERMISSIONS: risk_manager expands assets:*; the read
...
ROLE_ASSET_GRANTS: dict[str, tuple[str, ...]] = {
    "risk_manager": ("assets:read", "assets:write", "assets:delete"),
    "actuarial": ("assets:read",),
    "compliance": ("assets:read",),
    "internal_audit": ("assets:read",),
    "department_head": ("assets:read",),
...
def upgrade() -> None:
    ...
    # Ensure the asset permission rows exist (idempotent).
    ...
    # the explicit asset rows as well (18c1d2e3f4a6/18c1d2e3f4a7 precedent).
    ...
    for role_name, permission_keys in ROLE_ASSET_GRANTS.items():
