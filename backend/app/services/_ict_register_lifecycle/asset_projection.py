...
def _optional_engine_code(
    mapping: dict[str, str], value: str | None, *, field: str
) -> str | None:
    if value is None:
        return None
    ...
    return block.model_copy(
        update={
            ...
            "cif": _required_engine_code(
                _ASSET_BOOLEAN_CODE_BY_ENGINE_VALUE, block.cif, field="cif"
            ),
            "spof": _required_engine_code(
                _ASSET_BOOLEAN_CODE_BY_ENGINE_VALUE, block.spof, field="spof"
            ),
            ...
            "legacy": _required_engine_code(
                _ASSET_BOOLEAN_CODE_BY_ENGINE_VALUE, block.legacy, field="legacy"
            ),
        }
    )


async def load_primary_process_ids(
    db: AsyncSession, asset_ids: list[int], *, current_user: User
) -> dict[int, int]:
...
async def load_asset_derived_blocks(
    ...
    blocks = {
        asset.id: _canonical_asset_derived_block(derivation.assets[asset.id])
        for asset in assets
    }
...
async def _viewer_filtered_asset_graph(
    ...
    *,
    ...
    candidate_process_ids = {
        ...
        if link.asset_id in target_asset_ids
    }
    ...
    visible_process_ids = (
        set((await db.execute(process_query)).scalars().all())
        if candidate_process_ids
        else set()
    )

    candidate_supporting_ids = {
        link.supporting_asset_id
        for link in graph.asset_asset_links
        if link.dependent_asset_id in target_asset_ids
    }
    ...
    visible_supporting_ids = (
        set((await db.execute(supporting_query)).scalars().all())
    ...
    return replace(
        graph,
        processes=tuple(
            process for process in graph.processes if process.id in visible_process_ids
        ),
        assets=tuple(
            asset for asset in graph.assets if asset.id in retained_asset_ids
        ),
        process_asset_links=tuple(
            link
            for link in graph.process_asset_links
            if link.asset_id in target_asset_ids
            and link.process_id in visible_process_ids
        ),
        asset_asset_links=tuple(
            link
            for link in graph.asset_asset_links
            if link.dependent_asset_id in target_asset_ids
            and link.supporting_asset_id in visible_supporting_ids
        ),
        asset_vendor_links=tuple(
            link
            for link in graph.asset_vendor_links
            if link.asset_id in target_asset_ids
            and link.vendor_id in readable_vendor_ids
        ),
        process_vendor_links=tuple(
            link
            for link in graph.process_vendor_links
            if link.process_id in visible_process_ids
            and link.vendor_id in readable_vendor_ids
        ),
        vendors=tuple(
            vendor for vendor in graph.vendors if vendor.id in readable_vendor_ids
        ),
        contracts=tuple(
            contract
            for contract in graph.contracts
            if contract.vendor_id in readable_vendor_ids
        ),
        sub_outsourcing=tuple(
            entry
            for entry in graph.sub_outsourcing
            if entry.vendor_id in readable_vendor_ids
        ),
    )
...
async def _filter_asset_linked_context(
    ...
    *,
    ...
    visible_processes = {
        process.id: process
        for process in (await db.execute(process_query)).scalars().all()
    }
    ...
    supporting_assets = {
        asset.id: asset
        for asset in (await db.execute(supporting_query)).scalars().all()
    }
    ...
    visible_vendors = {
        vendor.id: vendor
        for vendor in (
            await db.execute(select(Vendor).where(Vendor.id.in_(readable_vendor_ids)))
        ).scalars().all()
    }
    ...
    for asset in assets:
        ...
        visible_process_rows = [
            ...
            if row.asset_id == asset.id and row.process_id in visible_processes
        ]
        ...
        visible_supporting_assets = [
            ...
            if row.dependent_asset_id == asset.id
            and row.supporting_asset_id in supporting_assets
        ]
        visible_vendor_rows = [
            row
            for row in vendor_rows
            if row.asset_id == asset.id and row.vendor_id in visible_vendors
        ]
        filtered_inputs = block.inputs.model_copy(
            update={
                "primary_process_id": visible_primary.id if visible_primary is not None else None
            }
        )
        filtered[asset.id] = block.model_copy(
            update={
                "primary_process_name": (
                    process_display_name(
                        visible_primary.l1_process, visible_primary.l2_subprocess
                    )
                ...
                "inherited_rto_hours": (
                    block.inherited_rto_hours if visible_primary is not None else None
                ),
                "cif_process_count": len(visible_cif_processes),
                "cif_process_names": [
                    process_display_name(process.l1_process, process.l2_subprocess)
                    for process in visible_cif_processes
                ],
                ...
                "vendor_names": [
                    visible_vendors[row.vendor_id].name for row in visible_vendor_rows
                ],
                "ict_service_codes": [row.ict_service_code for row in visible_vendor_rows],
                "contract_references": [
                    row.contract_reference
                    for row in visible_vendor_rows
                    if row.contract_reference is not None
                ],
...
def serialize_asset_detail(
    ...
    *,
    ...
    department_projection = (
        AssetDepartmentRead(name=department.name, code=department.code)
        if department is not None
        else None
    )
    business_owner_valid = bool(
        asset.business_owner is not None and asset.business_owner.is_active
    )
    ...
    if orphaned_roles:
        ...
        asset.business_owner is None
        or asset.ict_owner is None
...
async def serialize_asset_detail_with_primary(
    ...
    *,
    ...
    primary_map = await load_primary_process_ids(
        db, [asset.id], current_user=current_user
    )
...
async def serialize_asset_list(
    ...
    *,
    ...
    primary_map = await load_primary_process_ids(
        db, [asset.id for asset in assets], current_user=current_user
    )
