...
- `links.py` - Process<->Asset, Asset<->Asset, and Asset<->Vendor Link
  relation routes managed from the Asset detail (add/edit/remove, incl. the
  primary-Process designation swap and the S-code-typed vendor links); the
  Process-end reads live in `endpoints/processes/links.py` and the
  Vendor-end reads in `endpoints/vendors/links.py`

## Conventions

- Thin adapters: `require_permission("assets", ...)` dependencies only;
  no commits (ADR-002) — transactions live in
  `app/services/_ict_register_lifecycle/asset_lifecycle.py`,
  `app/services/_ict_register_lifecycle/asset_links.py`, and
