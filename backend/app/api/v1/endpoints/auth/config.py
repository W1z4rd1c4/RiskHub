...
from app.core.external_identity_policy import sso_enabled as profile_sso_enabled
from app.schemas.auth import AuthConfigResponse, AuthSsoConfig, DemoPersonaRead
from app.services.identity_capabilities import identity_config
...
async def get_auth_config(settings: Settings = Depends(get_settings)) -> AuthConfigResponse:
    sso_intended = profile_sso_enabled(settings)
    tenant_id = (settings.entra_tenant_id or None) if sso_intended else None
    client_id = (settings.entra_client_id or None) if sso_intended else None
    ...
    return AuthConfigResponse(
        identity=identity_config(settings),
        auth_mode=settings.auth_mode,
        demo_login_enabled=demo_login_enabled,
        password_login_enabled=password_login_enabled,
        local_mfa_policy=(
            settings.local_mfa_policy
            if settings.auth_mode == "password" and settings.directory_provider == "none"
            else None
        ),
